...
MooCountdown.Date = new Class({
  ...
  getDate : function(){
      ...
      //if( this.hours > 0 ) {
          ...
          new Element('span',{'html': sprintf('%02d', this.hours), 'class':'number'}).inject(elHours);
      ...
      //if( this.minutes > 0 ) {
          var elMinutes = new Element('span',{
            'id' : 'minutes',
            'html' : "<span class=\"number\">" + sprintf('%02d', this.minutes) + "</span><span class=\"text\">"+this.options.text[3]+"</span>",
            'class': 'countdown_box'
          }).inject(this.Container);
      ...
      new Element('span',{'html': sprintf('%02d', this.seconds), 'class':'number'}).inject(elSeconds);
...
MooCountdown.Simple = new Class({
  
  Extends: MooCountdown.Date,
  ...
  init: function() {
    this.parent();
    this.options.date = this.Container.get('text');
  }
