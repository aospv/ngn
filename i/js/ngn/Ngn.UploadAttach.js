...
Ngn.UploadAttach = new Class({
  
  initialize: function(list, attachs, _options) {
    ...
    var options = {
      ...
      onFileError: function(file) {
        file.ui.cancel.set('class', file.ui.cancel.get('class').replace('delete', 'cleanup'));
        file.ui.cancel.set('title', MooTools.lang.get('FancyUpload', 'retry'));
        file.ui.cancel.removeEvents().addEvent('click',
          function() {
            file.requeue();
            return false;
          });
      ...
      onFileRequeue: function(file) {
        ...
        file.ui.cancel.set('class', file.ui.cancel.get('class').replace('cleanup', 'delete'));
        file.ui.cancel.set('title', MooTools.lang.get('FancyUpload', 'cancel'));
        file.ui.cancel.removeEvents().addEvent('click',
          function() {
            file.remove();
            return false;
    ...
    this.uploader = new FancyUpload3.Attach(list, attachs, options);
    ...
    if (options.loadedFiles) {
      ...
      for (var i=0; i<options.loadedFiles.length; i++) {
        ...
        ui[i].element.adopt(
          ui[i].title,
          ui[i].size
        ).inject(this.uploader.list);
      }
      this.uploader.loadedFilesUi = ui;
    }
  }
