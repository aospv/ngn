...
class Db extends DbSimple_Mysql {
  ...
  public function getCell2($table, $field, DbCond $cond) {
  	return db()->selectCell("SELECT $field FROM $table".$cond->all());
  }
  
  public function increment($table, array $unicData, array $data = array(), $field = 'cnt') {
    $cond = DbCond::get();
    foreach ($unicData as $k => $v) $cond->addF($k, $v);
    if ($this->getCell2($table, $field, $cond)) {
      $this->query("UPDATE $table SET $field=$field+1".
        (empty($data) ? '' : ', ?a').$cond->all(), $data);
    } else {
