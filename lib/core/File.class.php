...
class File {
  ...
  static public function getMime($file) {
    ...
    finfo_close($r);
    LogWriter::v('FILEINFO_MIME', array($file, $info));
...
application/x-httpd-php-source  phps
  ...
  static public function checkExists($file) {
    if (!file_exists($file))
      throw new NgnException("File '$file' not exists");
  }
  
  static public function prepend($file, $contents) {
    file_put_contents($file, $contents.file_get_contents($file));
  }
  
  static public function findContents($file, $find) {
    return strstr(file_get_contents($file), $find);
  }
  
  static public function replace($file, $from, $to) {
    file_put_contents($file, str_replace($from, $to, file_get_contents($file)));
  }
  
  static public function replaceTttt($file, array $data) {
    file_put_contents($file, St::tttt(file_get_contents($file), $data));
  }
  
  static public function relative($path, $compareTo) {
  ...
  static public function lines($file) {
