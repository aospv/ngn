...
class PowerGraphic {
    ...
    function PowerGraphic()
    {
        ...
        $this->integer_y = ((isset($_GET['integer_y'])) && ($_GET['integer_y'] == 1)) ? true : false;
        ...
        $this->height_title         = (!empty($this->title)) ? ($this->string_height(2) + 15) : 0;
    ...
    function start()
    {
        ...
        if (($this->graphi?_exists == true)  &&  ((!empty($this->graphic_1)) || (!empty($this->graphic_2))))
        {
        ...
        $this->space_between_bars += ($this->graphi?_exists == true) ? 10 : 0;
    ...
    function create_graphic()
    {
        ...
        {
            $titleFontSize = 2;
            $center = ($this->width / 2) - ($this->string_width($this->title, $titleFontSize) / 2);
            imagestring($this->img, $titleFontSize, $center, 10, $this->title, $this->color['title']);
        }
        ...
        if (ereg("^(1|3|4)$", $this->type))
        {
            ...
            imagestring($this->img, 2, ($this->graphic_area_x1-$less-7), ($higher_value_y-7), $this->higher_value_str, $this->color['axis_values']);
            ...
            for ($i = 1; $i < 10; $i++)
            {
                ...
                if ($i % 2 == 0) {
                    ...
                    $less = 7 * strlen($value);
                    imagestring($this->img, 2, ($x1-$less-7), ($y2-7), $value, $this->color['axis_values']);
                }
            }

            // Axis X
            imagestring($this->img, 2, $this->graphic_area_x2+10, $this->graphic_area_y2+3, $this->axis_x, $this->color['title']);
            ...
            // Axis Y
            imagestring($this->img, 2, 20, $this->graphic_area_y1-20, $this->axis_y, $this->color['title']);
        ...
        if ($this->type == 1)
        {
            ...
            foreach ($this->x as $i => $parameter)
            {
                if (isset($this->z[$i])) {
                    ...
                    imageline($this->img, ($x1+1), ($y1-1), $x2, ($y1-1), $this->color['bar?_shadow']);
                    imageline($this->img, ($x2+1), ($y1-1), ($x2+1), $y2, $this->color['bar?_shadow']);
                    imageline($this->img, ($x2+2), ($y1-1), ($x2+2), $y2, $this->color['bar?_shadow']);
                ...
                imagestring($this->img, 1, ((($x1+$x2+15)/2) - (strlen($parameter)*7/2)), ($y2+$alt+2), $parameter, $this->color['axis_values']);
            }
        }
        ...
        else if ($this->type == 2)
        {
            ...
            foreach ($this->x as $i => $parameter)
            {
                if (isset($this->z[$i])) {
                    ...
                    $x2   = $x1 + $size;
                    $y2   = $y1 + 15;
                    imageline($this->img, ($x1), ($y2+1), $x2, ($y2+1), $this->color['bar?_shadow']);
                    imageline($this->img, ($x1), ($y2+2), $x2, ($y2+2), $this->color['bar?_shadow']);
                    imageline($this->img, ($x2+1), ($y1+1), ($x2+1), ($y2+2), $this->color['bar?_shadow']);
                    imagefilledrectangle($this->img, $x1, $y1, $x2, $y2, $this->color['bars_2']);
                    imagestring($this->img, 3, ($x2+7), ($y1+7), $this->number_formated($this->z[$i], 2), $this->color['bar?_shadow']);
                }
        ...
        */
        else if (ereg("^(3|4)$", $this->type))
        {
            ...
            foreach ($this->x as $i => $parameter)
            {
                ...
                imagestring($this->img, 1, ($x[$i] + 6 - (strlen($parameter)*7/2 )), ($this->graphic_area_y2+$alt+2), $parameter, $this->color['axis_values']);
    ...
    function calculate_height()
    {
        switch ($this->type)
        {
            // Vertical bars
            case 1:
                $this->legend_box_height   = ($this->graphi?_exists == true) ? 40 : 0;
    ...
    {
        if ($this->integer_y) return $number;
        ...
        switch ($this->skin)
        {
            ...
            case 1:
                ...
                if (ereg("^(1|2)$", $this->type))
                {
                    ...
                    $this->color['bar?_shadow'] = imagecolorallocate($this->img, 120, 170,  70);
                }
                else if (ereg("^(3|4)$", $this->type))
                {
                ...
                else if (ereg("^(5|6)$", $this->type))
                {
                    ...
                    $this->color['ar?_shadow'] = imagecolorallocate($this->img,  60, 110, 170);
                    $this->color['ar?_shadow'] = imagecolorallocate($this->img, 120, 170,  70);
                    $this->color['ar?_shadow'] = imagecolorallocate($this->img, 180, 120,  70);
                    $this->color['ar?_shadow'] = imagecolorallocate($this->img, 170, 100, 100);
                    $this->color['ar?_shadow'] = imagecolorallocate($this->img, 180, 180, 110);
                    $this->color['ar?_shadow'] = imagecolorallocate($this->img, 160, 110, 190);
                    $this->color['ar?_shadow'] = imagecolorallocate($this->img, 140, 140, 100);
                }
                break;
            ...
            case 2:
                ...
                if (ereg("^(1|2)$", $this->type))
                {
                    ...
                    $this->color['bar?_shadow'] = imagecolorallocate($this->img, 220, 220, 220);
                }
                else if (ereg("^(3|4)$", $this->type))
                {
                ...
                else if (ereg("^(5|6)$", $this->type))
                {
                    ...
                    $this->color['ar?_shadow'] = imagecolorallocate($this->img, 200, 220, 200);
                    $this->color['ar?_shadow'] = imagecolorallocate($this->img, 160, 200, 160);
                    $this->color['ar?_shadow'] = imagecolorallocate($this->img, 135, 180, 135);
                    $this->color['ar?_shadow'] = imagecolorallocate($this->img, 115, 160, 115);
                    $this->color['ar?_shadow'] = imagecolorallocate($this->img, 100, 140, 100);
                    $this->color['ar?_shadow'] = imagecolorallocate($this->img,  90, 120,  90);
                    $this->color['ar?_shadow'] = imagecolorallocate($this->img,  85, 100,  85);
                }
                break;
            ...
            case 3:
                ...
                if (ereg("^(1|2)$", $this->type))
                {
                    ...
                    $this->color['bar?_shadow'] = imagecolorallocate($this->img, 180, 150,   0);
                }
                else if (ereg("^(3|4)$", $this->type))
                {
            ...
            // Dying.ru
            case 4:
                $this->color['title']       = imagecolorallocate($this->img, 132, 119, 91);
                $this->color['background']  = imagecolorallocate($this->img,   0,   0,   0);
                $this->color['axis_values'] = imagecolorallocate($this->img, 126, 157, 47);
                $this->color['axis_line']   = imagecolorallocate($this->img, 132, 119, 91);
                $this->color['bg_lines']    = imagecolorallocate($this->img, 37, 33, 22);
                $this->color['bg_legend']   = imagecolorallocate($this->img,  70,  70,  70);
                ...
                {
                    $this->color['line']   = imagecolorallocate($this->img, 236, 212, 107);
                    $this->color['line_2'] = imagecolorallocate($this->img,   0, 180,   0);
                }
                else if (ereg("^(5|6)$", $this->type))
                {
...
function add_parameter() {
    ...
    display_status = (document.form_data.graphi?_values.checked == true) ? "" : "none";
...
function show_graphi?_values() {
    public display_status = (document.form_data.graphi?_values.checked == true) ? "" : "none";
    document.getElementById("value_2").style.display = display_status;
    for (public x = 0; x <= i; x++) {
...
'  </select>
  ...
  <div style="margin-top: 20px;" id="parameters">
    <div style="margin-bottom: 20px;">
      <input type="checkbox" name="graphi?_values" id="graphi?_values" value="1" onclick="javascript: show_graphi?_values();" style="border-width: 0px;" /> Show values for Graphic 2
    </div>
...
'    <div id="add_parameter"></div>
  ...
  <input type="submit" value="Create" style="cursor: pointer; margin: 10px 0px 0px 60px;" />
    ...
    function create_query_string() {
        ...
        $graphic['integer_y']   = $this->integer_y;
